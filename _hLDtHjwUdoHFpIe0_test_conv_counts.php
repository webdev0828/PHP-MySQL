...
//TODO: update conv per affiliate and promo tool start - add + 1 to conv per promo tool on conversion !important - promo tools add $src1 variable in clickid , 1 - banners, 3 - smart links, 4 pops, 5 - backoffers
//TODO2: now this works. In all promo tools links for clickid have to add $src1 = promo tool code, e.g. 3 for smartlinks and $src2 = promo tool id - for banners get from the link, then apply to all sales reporting scripts
// in promo.php $set = src1, $link = src2;
// update conversions per aff and promo tool start
$tidove=$_GET['tids'];
$pieces=explode("RkwuK6", $tidove);
$src1=$pieces[4] 
$src2=$pieces[5]
    if ($payout != 0) {
        if ($src1 == 1) {
        ...
        $data = array($src2);
        $statement = $db->prepare('update idevaff_' . $table . ' set conv = conv+1 where ' . $col . ' = ?');
        $statement->execute($data);
    }

    if ($payout != 0) {
        $data = array($idev);
        $statement = $db->prepare('update idevaff_affiliates set conv = conv+1 where id = ?');
        $statement->execute($data);
    }
// update conversions per aff and promo tool end
    if ($type != 3) {
        ...
        if (($setme == 1) && ($payout == 0)) {
            ...
            $statement = $db->prepare('insert into idevaff_sales (id, payment, approved, ip, code, tracking, amount, op1, op2, op3, type, split, profile, delay, referring_url, sub_id, tid1, tid2, tid3, tid4, target_url, currency, converted_amount, geo, tracking_method, conversion_time,alt_amt,sliding_scale,sliding_alternate,per_product) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');     
if (isset($_GET['tids']) || isset($_GET['geo'])) { 
$tidove=$_GET['tids'];
$pieces=explode("RkwuK6", $tidove);
$commission_country_code=$_GET['geo'];
            $data = array($idev, $payout, $setme, $ip_addr, $commission_time, $idev_ordernum, $avar, $ov1, $ov2, $ov3, $type, $split, $profile, $delay_insert, $r_url, $sub_id, $pieces[0], $pieces[1], $pieces[2], $pieces[3], $target_url, $currency_to_write, $converted_amount, $commission_country_code, $tracking_method_used, $diff, $alternate_amount, $sliding_scale, $sliding_alternate, $per_product_used);
} else {
