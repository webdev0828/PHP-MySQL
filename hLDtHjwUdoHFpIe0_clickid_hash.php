...
// update conversions per aff and promo tool start
    // TODO: get an array of columns data by clickid and use throughout script
if (isset($_GET['clickid'])){$clickid=$_GET['clickid'];}
        $trackers = $db->query("SELECT acct_id, src1, src2, sub_id, tid1, tid2, tid3, tid4, geo FROM idevaff_iptracking WHERE clickid = ?");
        $trackers = $get_update_record->fetch();
        $trackers = $trackers['column_name'];
    // TODO: get an array of columns data by clickid and use throughout script
// else if isset tids || if $tidove contains RkwuK6 proceed with old method
$tidove=$_GET['tids'];
