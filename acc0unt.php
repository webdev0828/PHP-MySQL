...
$checkfeed = $db->query("select stamp from idevaff_notes where note_display = 1 order by stamp desc limit 1");
$rescheck = $checkfeed->fetch();
$last_note_stamp = $rescheck['stamp'];
$smarty->assign('last_note_stamp', $last_note_stamp);
...
if ('1' === $page_request) {
    ...
    $now = time();
    $qHotLinks = $db->query("SELECT idevaff_groups.inhouse,idevaff_links.id, idevaff_links.grp, idevaff_groups.name, idevaff_links.linktext, idevaff_links.land, ROUND(idevaff_links.earnings / idevaff_links.hits * 100, 4) AS epc, ROUND(idevaff_links.conv / idevaff_links.hits * 100, 3) AS cr FROM idevaff_links INNER JOIN idevaff_groups ON idevaff_links.grp = idevaff_groups.id WHERE idevaff_groups.show_in_table = 1 AND idevaff_groups.niche LIKE '%mainstream%' ORDER BY epc DESC, cr DESC LIMIT 9");
    $qFHotBanners = $db->query("SELECT idevaff_groups.inhouse,idevaff_banners.number, idevaff_banners.grp, idevaff_groups.name, idevaff_banners.description, idevaff_banners.size1, idevaff_banners.size2, idevaff_banners.language, ROUND(idevaff_banners.earnings / idevaff_banners.hits * 100, 4) AS epc, ROUND(idevaff_banners.conv / idevaff_banners.hits * 100, 3) AS cr FROM idevaff_banners INNER JOIN idevaff_groups ON idevaff_banners.grp = idevaff_groups.id WHERE idevaff_groups.show_in_table = 1 AND idevaff_groups.niche LIKE '%mainstream%' ORDER BY epc DESC, cr DESC LIMIT 9");
    $qHotSmartLinks = $db->query("SELECT idevaff_groups.inhouse,idevaff_links.id, idevaff_links.grp, idevaff_groups.name, ROUND(idevaff_links.earnings / idevaff_links.hits * 100, 4) AS epc, ROUND(idevaff_links.conv / idevaff_links.hits * 100, 3) AS cr FROM idevaff_links INNER JOIN idevaff_groups ON idevaff_links.grp = idevaff_groups.id WHERE idevaff_groups.name LIKE '%smartlink%' AND idevaff_groups.niche LIKE '%mainstream%' ORDER BY epc DESC, cr DESC LIMIT 9");
    $qHotPopUnders = $db->query("SELECT idevaff_groups.inhouse,idevaff_htmlads.id, idevaff_htmlads.grp, idevaff_htmlads.name, ROUND(idevaff_htmlads.earnings / idevaff_htmlads.hits * 100, 4) AS epc, ROUND(idevaff_htmlads.conv / idevaff_htmlads.hits * 100, 3) AS cr FROM idevaff_htmlads INNER JOIN idevaff_groups ON idevaff_htmlads.grp = idevaff_groups.id WHERE idevaff_groups.name LIKE '%popunder%' AND idevaff_groups.niche LIKE '%mainstream%' ORDER BY epc DESC, cr DESC LIMIT 9");
    $qHotBackOffers = $db->query("SELECT idevaff_groups.inhouse,idevaff_email_templates.id, idevaff_email_templates.grp, idevaff_email_templates.name, ROUND(idevaff_email_templates.earnings / idevaff_email_templates.hits * 100, 4) AS epc, ROUND(idevaff_email_templates.conv / idevaff_email_templates.hits * 100, 3) AS cr FROM idevaff_email_templates INNER JOIN idevaff_groups ON idevaff_email_templates.grp = idevaff_groups.id WHERE idevaff_email_templates.name LIKE '%backoffer%' AND idevaff_groups.niche LIKE '%mainstream%' ORDER BY epc DESC, cr DESC LIMIT 9");

    $qaHotLinks = $db->query("SELECT idevaff_groups.inhouse,idevaff_links.id, idevaff_links.grp, idevaff_groups.name, idevaff_links.linktext, idevaff_links.land, ROUND(idevaff_links.earnings / idevaff_links.hits * 100, 4) AS epc, ROUND(idevaff_links.conv / idevaff_links.hits * 100, 3) AS cr FROM idevaff_links INNER JOIN idevaff_groups ON idevaff_links.grp = idevaff_groups.id WHERE idevaff_groups.show_in_table = 1 AND idevaff_groups.niche LIKE '%adult%' ORDER BY epc DESC, cr DESC LIMIT 9");
    $qaFHotBanners = $db->query("SELECT idevaff_groups.inhouse,idevaff_banners.number, idevaff_banners.grp, idevaff_groups.name, idevaff_banners.description, idevaff_banners.size1, idevaff_banners.size2, idevaff_banners.language, ROUND(idevaff_banners.earnings / idevaff_banners.hits * 100, 4) AS epc, ROUND(idevaff_banners.conv / idevaff_banners.hits * 100, 3) AS cr FROM idevaff_banners INNER JOIN idevaff_groups ON idevaff_banners.grp = idevaff_groups.id WHERE idevaff_groups.show_in_table = 1 AND idevaff_groups.niche LIKE '%adult%' ORDER BY epc DESC, cr DESC LIMIT 9");
    $qaHotSmartLinks = $db->query("SELECT idevaff_groups.inhouse,idevaff_links.id, idevaff_links.grp, idevaff_groups.name, ROUND(idevaff_links.earnings / idevaff_links.hits * 100, 4) AS epc, ROUND(idevaff_links.conv / idevaff_links.hits * 100, 3) AS cr FROM idevaff_links INNER JOIN idevaff_groups ON idevaff_links.grp = idevaff_groups.id WHERE idevaff_groups.name LIKE '%smartlink%' AND idevaff_groups.niche LIKE '%adult%' ORDER BY epc DESC, cr DESC LIMIT 9");
    $qaHotPopUnders = $db->query("SELECT idevaff_groups.inhouse,idevaff_htmlads.id, idevaff_htmlads.grp, idevaff_htmlads.name, ROUND(idevaff_htmlads.earnings / idevaff_htmlads.hits * 100, 4) AS epc, ROUND(idevaff_htmlads.conv / idevaff_htmlads.hits * 100, 3) AS cr FROM idevaff_htmlads INNER JOIN idevaff_groups ON idevaff_htmlads.grp = idevaff_groups.id WHERE idevaff_groups.name LIKE '%popunder%' AND idevaff_groups.niche LIKE '%adult%' ORDER BY epc DESC, cr DESC LIMIT 9");
    $qaHotBackOffers = $db->query("SELECT idevaff_groups.inhouse,idevaff_email_templates.id, idevaff_email_templates.grp, idevaff_email_templates.name, ROUND(idevaff_email_templates.earnings / idevaff_email_templates.hits * 100, 4) AS epc, ROUND(idevaff_email_templates.conv / idevaff_email_templates.hits * 100, 3) AS cr FROM idevaff_email_templates INNER JOIN idevaff_groups ON idevaff_email_templates.grp = idevaff_groups.id WHERE idevaff_email_templates.name LIKE '%backoffer%' AND idevaff_groups.niche LIKE '%adult%' ORDER BY epc DESC, cr DESC LIMIT 9");
    ...
    $qRunningPopUnders = $db->query('SELECT distinct grp FROM idevaff_htmlads WHERE id IN (SELECT src2 FROM idevaff_iptracking WHERE acct_id = '.$linkid.' AND src1 = 4 AND stamp between '.$weekago.' and '.$now.' GROUP BY src2 HAVING COUNT(*) >= 1)');
    $qRunningBackOffers = $db->query('SELECT distinct grp FROM idevaff_email_templates WHERE id IN (SELECT src2 FROM idevaff_iptracking WHERE acct_id = '.$linkid.' AND src1 = 5 AND stamp between '.$weekago.' and '.$now.' GROUP BY src2 HAVING COUNT(*) >= 1)');

    $hotlinks = $qHotLinks->fetchAll(PDO::FETCH_ASSOC);
    $hotbanners = $qFHotBanners->fetchAll(PDO::FETCH_ASSOC);
    $hotsmartlinks = $qHotSmartLinks->fetchAll(PDO::FETCH_ASSOC);
    $hotpopunders = $qHotPopUnders->fetchAll(PDO::FETCH_ASSOC);
    $hotbackoffers = $qHotBackOffers->fetchAll(PDO::FETCH_ASSOC);

    $hotalinks = $qaHotLinks->fetchAll(PDO::FETCH_ASSOC);
    $hotabanners = $qaFHotBanners->fetchAll(PDO::FETCH_ASSOC);
    $hotasmartlinks = $qaHotSmartLinks->fetchAll(PDO::FETCH_ASSOC);
    $hotapopunders = $qaHotPopUnders->fetchAll(PDO::FETCH_ASSOC);
    $hotabackoffers = $qaHotBackOffers->fetchAll(PDO::FETCH_ASSOC);

    $runninglinks = $qRunningLinks->fetchAll();
    $runningbanners = $qRunningBanners->fetchAll();
    $runningpopunders = $qRunningPopUnders->fetchAll();
    $runningbackoffers = $qRunningBackOffers->fetchAll();
    ...
    $runningPopUndersArray=[];
    foreach ($runningpopunders as $runningpopunder){
        $runningPopUndersArray[]=$runningpopunder['grp'];
    }
    $runningBackOffersArray=[];
    foreach ($runningbackoffers as $runningbackoffer){
        $runningBackOffersArray[]=$runningbackoffer['grp'];
    }

    $smarty->assign('hotlinks', $hotlinks);
    $smarty->assign('hotbanners', $hotbanners);
    $smarty->assign('hotsmartlinks', $hotsmartlinks);
    $smarty->assign('hotpopunders', $hotpopunders);
    $smarty->assign('hotbackoffers', $hotbackoffers);

    $smarty->assign('hotalinks', $hotalinks);
    $smarty->assign('hotabanners', $hotabanners);
    $smarty->assign('hotasmartlinks', $hotasmartlinks);
    $smarty->assign('hotapopunders', $hotapopunders);
    $smarty->assign('hotabackoffers', $hotabackoffers);

    $smarty->assign('runninglinks', $runningLinksArray);
    $smarty->assign('runningbanners', $runningBannersArray);
    $smarty->assign('runningpopunders', $runningPopUndersArray);
    $smarty->assign('runningbackoffers', $runningBackOffersArray);
...
//  STATS
    ...
    if ('49' === $page_request) {
        $data_get = '49';
        $smarty->assign('internal_page', 49);
    }
}
