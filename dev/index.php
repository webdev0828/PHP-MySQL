...
while (1 <= $l) {
	$commission = $db->query('SELECT COUNT(*) FROM idevaff_archive WHERE bonus = \'0\' AND DATE_FORMAT(FROM_UNIXTIME(code), \'%m\') = ' . date('m', strtotime('-' . $l . ' months')));
	$payouts = $db->query('SELECT COALESCE(SUM(payment),0) FROM idevaff_archive WHERE bonus = \'0\' /*AND tracking NOT LIKE \'%fictive%\'*/ /*AND top_tier_tag = \'0\'*/ AND DATE_FORMAT(FROM_UNIXTIME(code), \'%m\') = ' . date('m', strtotime('-' . $l . ' months')));
	// $ref_payouts = $db->query('SELECT COALESCE(SUM(payment),0) FROM idevaff_archive WHERE bonus = \'0\' AND top_tier_tag = \'1\' AND DATE_FORMAT(FROM_UNIXTIME(code), \'%m\') = ' . date('m', strtotime('-' . $l . ' months')));
	$monthly_commissions[$i++] = array('name' => date('M', strtotime('-' . $l . ' months')), 'commissions' => $commission->fetchColumn(), 'payouts' => $payouts->fetchColumn()/*, 'ref_payouts' => $ref_payouts->fetchColumn()*/);
	--$l;
}
