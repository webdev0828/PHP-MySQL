...
$pieces=explode("RkwuK6", $geotidove);
$src1=$pieces[5];
$src2=$pieces[6];
    if ($payout != 0) {
        if ($src1 == 1) {
...
if (isset($_GET['geotids'])) { 
$geotidove=$_GET['geotids'];
$pieces=explode("RkwuK6", $geotidove);
$commission_country_code = $pieces[0];
if ($sub_id !== '' && !empty($pieces[7]) ) {
$sub_id = $pieces[7];
}
            $data = array($idev, $payout, $setme, $ip_addr, $commission_time, $idev_ordernum, $avar, $ov1, $ov2, $ov3, $type, $split, $profile, $delay_insert, $r_url, $sub_id, $pieces[1], $pieces[2], $pieces[3], $pieces[4], $target_url, $currency_to_write, $converted_amount, $commission_country_code, $tracking_method_used, $diff, $alternate_amount, $sliding_scale, $sliding_alternate, $per_product_used, $src1, $src2);
} else {
