...
if ($report_number) {
      ...
      if ($paramSearch != '') {
        ...
        foreach ( $result->fetchAll() as $pqry) {
            ...
            if ( $aff_inv == 1) {
                $tmpcm[] = '<form method="POST" action="/invoice" target="_blank"><input name="csrf_token" value="'.$_SESSION['csrf_token'].'" type="hidden" /><input type="hidden" value="'.$record_id.'" name="stamp"><input type="submit" value="'.$invoice_button.'" class="btn btn-xs btn-primary"></form>';
            }
