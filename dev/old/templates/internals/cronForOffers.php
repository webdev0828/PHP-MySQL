...
// test end
		...
		if ( $pqry['niche'] == "Adult Cams" ) {
			$niche = "<strong class='fa-lg'>18+</strong> <i class='fa fa-camera fa-lg fa-fw' title='Adult Cams'></i>";
		} elseif ( $pqry['niche'] == "Adult Dating" ) {
			$niche = "<strong class='fa-lg'>18+</strong> <i class='fa fa-heart fa-lg fa-fw' title='Adult Dating'></i>";
		} elseif ( $pqry['niche'] == "Adult Nutra" ) {
			$niche = "<strong class='fa-lg'>18+</strong> <i class='fa fa-leaf fa-lg fa-fw' title='Adult Nutra'></i>";
		} elseif ( $pqry['niche'] == "Adult Video" ) {
			$niche = "<strong class='fa-lg'>18+</strong> <i class='fa fa-video fa-lg fa-fw' title='Adult Video'></i>";
		} elseif ( $pqry['niche'] == "Adult Apps" ) {
			$niche = "<strong class='fa-lg'>18+</strong> <i class='fa fa-compact-disc fa-lg fa-fw' title='Adult Apps'></i>";
...
// TODO:  simplify payout calcs without * 1
		...
		$tmpOfferList = array(
			'"placeholder" as placeholder'=>'<a href="javascript:void(0);" class="row-details-toggle details-control"></a>',
			'name'=>$name,
			'niche'=>$niche,
			'countries'=>$countries,
			'devices'=>$devices,
			...
			'carrier'=>strtolower( $carriers ) == 'all' ? $getlanguage['custom_all'] : $carriers,
			'flow_type'=>strtolower( $flow_type ) == 'revenue share' ? $getlanguage['custom_revenue_share'] : $flow_type,
			'payout'=>$payout,
			'id'=>$id
		);
...
//		}
//		else{
//			$nonPercentArray[]=$item;
//		}
//	}
//	echo "<pre>";
//	print_r($percentArray);
//	print_r($nonPercentArray);exit;
//	for($j=0;$j<count($percentArray);$j++) {
//		for ( $i = 0; $i < count( $percentArray ) - 1; $i ++ ) {
//			$a=$percentArray[$i]['payout'];
//			$b=$percentArray[$i+1]['payout'];
//			if(strpos($a, "%" ) === true){
//				if(strpos($b, "%" ) === true){
//					$a1=str_replace("%","",str_replace(" ","",str_replace(",","",$a)));
//					$a2=str_replace("%","",str_replace(" ","",str_replace(",","",$b)));
//					if(floatval($a1) < floatval($a2)) {
//						$temp = $percentArray[$i+1];
...
//		}
//	}
//	for($j=0;$j<count($nonPercentArray);$j++) {
//		for ( $i = 0; $i < count( $nonPercentArray ) - 1; $i ++ ) {
//			$a=$nonPercentArray[$i]['payout'];
//			$b=$nonPercentArray[$i+1]['payout'];
//			if(strpos($a,"-") === true){
//				$a1=explode("-",$a);
...
//					$temp = $nonPercentArray[$i+1];
//					$nonPercentArray[$i+1]=$nonPercentArray[$i];
//					$nonPercentArray[$i]=$temp;
//				}
//			}
//			else{
...
//	}
	...
	echo "<pre>";
//	$arr=array_merge($percentArray,$nonPercentArray);
	$output['aaData']=$array;
	print_r($array);exit;
	$jsonData=json_encode($output);
	file_put_contents('../source/common/js/offers2.json',$jsonData);
	echo "done";
	exit;
} catch (Exception $e) {
	echo $e->getMessage();
