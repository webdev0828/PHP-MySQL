                ...
                $.ajax({
                    ...
                    success: function (resp) {
                        ...
                        if(resp.link.length > 0) {
                            ...
                            <div class="row"><div class="col-md-4"><label>'+jstext.language.custom_filter_by_landing+'</label>' +
                                ...
                                '</select></div></div>'; // TODO: filter by country column from _links table + Filter only HOT
                        }
                        ...
                        $.each(resp.link,function (idx,data) {
                            ...
                            if(epc >= 0.3 && sr >= 0.3){
                                html+=' <i class="fa fa-link fa-fw"></i> <span class="label label-success">HOT</span> '+resp.data.name+' '+data.linktext+' '+data.country+' '+(data.land === 1 ? 'Landing' : 'Pre-landing')+' <span style="float:right">EPC: '+(data.hits === 0 ? (0).toFixed(4) : epc.toFixed(4))+' €</span> ';
                            }
                            else{
                                html+=' <i class="fa fa-link fa-fw"></i> '+resp.data.name+' '+data.linktext+' '+data.country+' '+(data.land === 1 ? 'Landing' : 'Pre-landing')+' <span style="float:right">EPC: '+(data.hits === 0 ? (0).toFixed(4) : epc.toFixed(4))+' €</span> ';
                            }
                            html +=('     </h4> \
                                    </div> \
                        ...
                        $.each(resp.banners,function (idx,data) {
                            ...
                            html += '</textarea>'
                            html += '<br/>'
                            html +=(' <label>'+jstext.language.marketing_target_url+':</label> <textarea style="font-weight:normal !improtant;" id="banner-direct-link-'+data.grp+'-'+data.number+'" rows="1" class="form-control">'+data.lnk+'</textarea>')
                            html += '</li>'
                            html += '</ul>'
                            html += '</div>'
                            html += '</div>'
        ...
        if ($('#dyntable_Stats').length > 0) {
            ...
            function format(d, type) {
                ...
                $.ajax({
                    ...
                    data: {startD: dates.startD, endD: dates.endD, info: infoT,aggregate:$('#aggregate').val()},
                    dataType: 'json',
                    cache: false,
...
function statChildFilter(trId, startD, endD, infoT){
    var trans = JSON.parse($("#trans").val());
    ...
    $.ajax({
        url: '/templates/internals/core_ajax_stats_detail.php',
        data: {startD: dates.startD, endD: dates.endD, info: infoT, country: country, subId: subId, tId1: tId1, tId2: tId2, tId3: tId3, tId4: tId4, combinations: combinations, smartTools: smartTools, offerTools: offerTools,aggregate:$('#aggregate').val()},
        dataType: 'json',
        cache: false,
        success: function (info) {
            // Countries
