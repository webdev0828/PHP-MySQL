...
if (isset($_GET['geosl'])) { $geosl=$_GET['geosl']; }
$pieces=explode("RkwuK6", $geosl);
$src1=$pieces[1];
$src2=$pieces[2];
    if ($payout != 0) {
        if ($src1 == 1) {
...
if (isset($_GET['tidp1'])) { $tidp1=$_GET['tidp1']; }
$tidp1pieces=explode("RkwuK6", $tidp1);
$tid1=$tidp1pieces[0];
$tid2=$tidp1pieces[1];
if (isset($_GET['tidp2'])) { $tidp2=$_GET['tidp2']; }
$tidp2pieces=explode("RkwuK6", $tidp2);
$tid3=$tidp2pieces[0];
$tid4=$tidp2pieces[1];
if (isset($_GET['geosl'])) { $geosl=$_GET['geosl']; }
$geoslpieces=explode("RkwuK6", $geosl);
$geo=$geoslpieces[0];
///
            ...
            $data = array($idev, $payout, $setme, $ip_addr, $commission_time, $idev_ordernum, $avar, $ov1, $ov2, $ov3, $type, $split, $profile, $delay_insert, $r_url, $sub_id, $tid1, $tid2, $tid3, $tid4, $target_url, $currency_to_write, $converted_amount, $geo, $tracking_method_used, $diff, $alternate_amount, $sliding_scale, $sliding_alternate, $per_product_used, $src1, $src2);
            $statement->execute($data);
