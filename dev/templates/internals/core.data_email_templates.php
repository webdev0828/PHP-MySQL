...
if (!defined('IDEV_FILE_AUTH')) { die('Unauthorized Access'); }
	...
	$now = time();
	$qRunningLinks = $db->query("SELECT distinct grp FROM idevaff_email_templates WHERE grp IN (select id from idevaff_groups where name LIKE '%backoffer%') and id IN (SELECT src2 FROM idevaff_iptracking WHERE acct_id = ".$linkid." AND src1 = 5 AND stamp between ".$weekago." and ".$now." GROUP BY src2 HAVING COUNT(*) >= 1)");
	$runninglinks = $qRunningLinks->fetchAll();
	$runningLinksArray=[];
	foreach ($runninglinks as $runninglink){
		$runningLinksArray[]=$runninglink['grp'];
	}
$smarty->assign('runningLinksArray', $runningLinksArray);
// -------------------------------------------------------------------------------------------------
...
$get_etemplates_links = $db->prepare("select * from idevaff_email_templates where name LIKE '%BackOffer%' and grp NOT IN (select id FROM idevaff_groups_exclude WHERE affiliate_id = ?) order by sort");
...
$groupname = $getad['name'];
		if (in_array($groupid,$runningLinksArray)) {
		$running = ' *';
		} else {
		$running = NULL;
		}
            $tmpht = array( 
                ...
                'etemplates_group_name' => $groupname.$running, 
            );
