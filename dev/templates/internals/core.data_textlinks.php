...
// TEXT LINKS
	...
	$now = time();
	$qRunningLinks = $db->query("SELECT distinct grp FROM idevaff_links WHERE grp IN (select id from idevaff_groups where name LIKE '%smartlink%') and id IN (SELECT src2 FROM idevaff_iptracking WHERE acct_id = ".$linkid." AND src1 = 3 AND stamp between ".$weekago." and ".$now." GROUP BY src2 HAVING COUNT(*) >= 1)");
...
$get_textlinks = $db->prepare("select * from idevaff_groups where name LIKE '%SmartLink%' and id NOT IN (select id FROM idevaff_groups_exclude WHERE affiliate_id = ?) and links > 0 order by name");
...
$groupname=$q['name'];
		...
		$running = NULL;
            ...
            $tmpt = array( 
                ...
                'textlink_group_name' => $groupname.$running,
            );
