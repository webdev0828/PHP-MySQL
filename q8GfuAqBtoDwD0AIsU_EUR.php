<?php
/*
Commissions reporting script. Works for the following partners: 
OneBip              - ACN
BongaCash           - works
ZetaNiche           - works
BrokerBabe          - works
...
TrafficPartner      - test
SlimSpots           - works
TrafficCompany      - works
BitterStrawberry    - works
Glize               - works
DatingFactory       - works
WarGaming           - works
AWE: TODO           - test
...
// TODO: check if clickid matches the acct_id?
// update conversions per aff and promo tool start
$clickid    = isset($_GET['clickid']) ? $_GET['clickid'] : '';
$qtrackers  = $db->prepare('SELECT src1, src2, sub_id, tid1, tid2, tid3, tid4, geo, ip FROM idevaff_iptracking WHERE clickid = ? LIMIT 1');
...
//  postback start
// GET DATA START
$record = $db->query("SHOW TABLE STATUS LIKE 'idevaff_sales'")->fetch(PDO::FETCH_ASSOC)['Auto_increment'] -1;
...
$chcek_get_duplicate = $db->prepare('select record from idevaff_postbacks_logs where record = ? and result = 1 and grp = ? and postback_url = ? and method = 0 and status = 1');
...
$chcek_post_duplicate = $db->prepare('select record from idevaff_postbacks_logs where record = ? and result = 1 and grp = ? and postback_url = ? and method = 1 and status = 1');
