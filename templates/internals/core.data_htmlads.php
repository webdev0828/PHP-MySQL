...
if (!defined('IDEV_FILE_AUTH')) { die('Unauthorized Access'); }
	...
	$now = time();
	$qRunningLinks = $db->query("SELECT distinct grp FROM idevaff_htmlads WHERE grp IN (select id from idevaff_groups where name LIKE '%popunder%') and id IN (SELECT src2 FROM idevaff_iptracking WHERE acct_id = ".$linkid." AND src1 = 4 AND stamp between ".$weekago." and ".$now." GROUP BY src2 HAVING COUNT(*) >= 1)");
...
$get_htmlads = $db->prepare("select * from idevaff_groups where name LIKE '%PopUnder%' and id NOT IN (select id FROM idevaff_groups_exclude WHERE affiliate_id = ?) and hads > 0 order by name");
...
$groupname=$qh['name'];
		...
		$running = NULL;
            ...
            $tmpht = array( 
                ...
                'htmlad_group_name' => $groupname.$running, 
            );
