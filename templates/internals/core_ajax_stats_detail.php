...
// TODO: add " - Fallback" to offer tools column names if converted geo is not in offer geo array.
...
function detailsByDateRange($startD, $endD, $current_date, $db, $id, $info,$country,$subId,$tId1,$tId2,$tId3,$tId4,$combinations,$smartTools,$offerTools,$grouping)
{
    ...
    $q_raw_visits_country = "select COUNT(*) v,geo n from idevaff_iptracking where acct_id = $id and stamp " . $date_where .$whereCond. " group by geo order by v desc";

//	echo $q_raw_visits_country;
	$q_raw_visits_sub_id = "select COUNT(*) v,sub_id n from idevaff_iptracking where acct_id = $id and sub_id IS NOT NULL AND sub_id <> ''  and stamp " . $date_where .$whereCond. " group by sub_id order by v desc";
	$q_raw_visits_tid1 = "select COUNT(*) v,tid1 n from idevaff_iptracking where acct_id = $id and tid1 IS NOT NULL AND tid1 <> ''  and stamp " . $date_where .$whereCond. " group by tid1 order by v desc";
	$q_raw_visits_tid2 = "select COUNT(*) v,tid2 n from idevaff_iptracking where acct_id = $id and tid2 IS NOT NULL AND tid2 <> '' and stamp " . $date_where .$whereCond. " group by tid2 order by v desc";
	$q_raw_visits_tid3 = "select COUNT(*) v,tid3 n from idevaff_iptracking where acct_id = $id and tid3 IS NOT NULL AND tid3 <> '' and stamp " . $date_where .$whereCond. " group by tid3 order by v desc";
	$q_raw_visits_tid4 = "select COUNT(*) v,tid4 n from idevaff_iptracking where acct_id = $id and tid4 IS NOT NULL AND tid4 <> '' and stamp " . $date_where .$whereCond. " group by tid4 order by v desc";
	$q_raw_visits_tid_combinations = "select COUNT(*) v,CONCAT(tid1,' ',tid2, ' ',tid3, ' ',tid4) n from idevaff_iptracking where acct_id = $id and CONCAT(tid1,tid2,tid3,tid4) IS NOT NULL AND CONCAT(tid1,tid2,tid3,tid4) <>'' and stamp " . $date_where .$whereCond. " group by n order by v desc";
	$q_raw_visits_src1 = "select COUNT(*) v,src1,src2 from idevaff_iptracking where acct_id = $id and src1 IS NOT NULL AND src1 <> '' and src2 IS NOT NULL AND src2 <> '' and stamp " . $date_where .$whereCond. " group by src1,src2 order by v desc";
//    echo $q_raw_visits_src1;exit;
	$q_unique_visits_country = "select COUNT(distinct(ip)) v,geo n from idevaff_iptracking where acct_id = $id  and stamp " . $date_where .$whereCond. " group by geo order by v desc";
	$q_unique_visits_sub_id = "select COUNT(distinct(ip)) v,sub_id n from idevaff_iptracking where acct_id = $id and sub_id IS NOT NULL AND sub_id <> '' and stamp " . $date_where .$whereCond. " group by sub_id order by v desc";
	$q_unique_visits_tid1 = "select COUNT(distinct(ip)) v,tid1 n from idevaff_iptracking where acct_id = $id and tid1 IS NOT NULL AND tid1 <> '' and stamp " . $date_where .$whereCond. " group by tid1 order by v desc";
	$q_unique_visits_tid2 = "select COUNT(distinct(ip)) v,tid2 n from idevaff_iptracking where acct_id = $id and tid2 IS NOT NULL AND tid2 <> '' and stamp " . $date_where .$whereCond. " group by tid2 order by v desc";
	$q_unique_visits_tid3 = "select COUNT(distinct(ip)) v,tid3 n from idevaff_iptracking where acct_id = $id and tid3 IS NOT NULL AND tid3 <> '' and stamp " . $date_where .$whereCond. " group by tid3 order by v desc";
	$q_unique_visits_tid4 = "select COUNT(distinct(ip)) v,tid4 n from idevaff_iptracking where acct_id = $id and tid4 IS NOT NULL AND tid4 <> '' and stamp " . $date_where .$whereCond. " group by tid4 order by v desc";
	$q_unique_visits_tid_combinations = "select COUNT(distinct(ip)) v,CONCAT(tid1,' ',tid2, ' ',tid3, ' ',tid4) n from idevaff_iptracking where acct_id = $id and CONCAT(tid1,tid2,tid3,tid4) IS NOT NULL AND CONCAT(tid1,tid2,tid3,tid4) <>'' and stamp " . $date_where .$whereCond. " group by n order by v desc";
	$q_unique_visits_src1= "select COUNT(distinct(ip)) v,src1,src2 from idevaff_iptracking where acct_id = $id and src1 IS NOT NULL AND src1 <> ''  and src2 IS NOT NULL AND src2 <> '' and stamp " . $date_where .$whereCond. " group by src1,src2 order by v desc";

	$q_earnings_country = "select COALESCE(SUM(payment),0) v,geo n from idevaff_sales where id = $id and approved=1 and top_tier_tag=0 and code " . $date_where .$whereCond. " group by geo order by v desc";
	$q_earnings_sub_id = "select COALESCE(SUM(payment),0) v,sub_id n from idevaff_sales where id = $id and approved=1 and top_tier_tag=0 and sub_id IS NOT NULL AND sub_id <> '' and code " . $date_where .$whereCond. " group by sub_id order by v desc";
	$q_earnings_tid1 = "select COALESCE(SUM(payment),0) v,tid1 n from idevaff_sales where id = $id and approved=1 and top_tier_tag=0 and tid1 IS NOT NULL AND tid1 <> '' and code " . $date_where .$whereCond. " group by tid1 order by v desc";
	$q_earnings_tid2 = "select COALESCE(SUM(payment),0) v,tid2 n from idevaff_sales where id = $id and approved=1 and top_tier_tag=0 and tid2 IS NOT NULL AND tid2 <> '' and code " . $date_where .$whereCond. " group by tid2 order by v desc";
	$q_earnings_tid3 = "select COALESCE(SUM(payment),0) v,tid3 n from idevaff_sales where id = $id and approved=1 and top_tier_tag=0 and tid3 IS NOT NULL AND tid3 <> '' and code " . $date_where .$whereCond. " group by tid3 order by v desc";
	$q_earnings_tid4 = "select COALESCE(SUM(payment),0) v,tid4 n from idevaff_sales where id = $id and approved=1 and top_tier_tag=0 and tid4 IS NOT NULL AND tid4 <> '' and code " . $date_where .$whereCond. " group by tid4 order by v desc";
	$q_earnings_tid_combinations = "select COALESCE(SUM(payment),0) v,CONCAT(tid1,' ',tid2, ' ',tid3, ' ',tid4) n from idevaff_sales where id = $id and CONCAT(tid1,tid2,tid3,tid4) IS NOT NULL AND CONCAT(tid1,tid2,tid3,tid4) <>'' and approved=1 and top_tier_tag=0 and code " . $date_where .$whereCond. " group by n order by v desc";
	$q_earnings_src1 = "select COALESCE(SUM(payment),0) v,src1,src2 from idevaff_sales where id = $id and approved=1 and top_tier_tag=0 and src1 IS NOT NULL AND src1 <> ''  and src2 IS NOT NULL AND src2 <> '' and code " . $date_where .$whereCond. " group by src1,src2 order by v desc";

	$q_paid_earnings_country = "select COALESCE(SUM(payment),0) v,geo n from idevaff_archive where id = $id and top_tier_tag=0 and code " . $date_where .$whereCond. " group by geo order by v desc";
	$q_paid_earnings_sub_id = "select COALESCE(SUM(payment),0) v,sub_id n from idevaff_archive where id = $id and top_tier_tag=0 and sub_id IS NOT NULL AND sub_id <> '' and code " . $date_where .$whereCond. " group by sub_id order by v desc";
	$q_paid_earnings_tid1 = "select COALESCE(SUM(payment),0) v,tid1 n from idevaff_archive where id = $id and top_tier_tag=0 and tid1 IS NOT NULL AND tid1 <> '' and code " . $date_where .$whereCond. " group by tid1 order by v desc";
	$q_paid_earnings_tid2 = "select COALESCE(SUM(payment),0) v,tid2 n from idevaff_archive where id = $id and top_tier_tag=0 and tid2 IS NOT NULL AND tid2 <> '' and code " . $date_where .$whereCond. " group by tid2 order by v desc";
	$q_paid_earnings_tid3 = "select COALESCE(SUM(payment),0) v,tid3 n from idevaff_archive where id = $id and top_tier_tag=0 and tid3 IS NOT NULL AND tid3 <> '' and code " . $date_where .$whereCond. " group by tid3 order by v desc";
	$q_paid_earnings_tid4 = "select COALESCE(SUM(payment),0) v,tid4 n from idevaff_archive where id = $id and top_tier_tag=0 and tid4 IS NOT NULL AND tid4 <> '' and code " . $date_where .$whereCond. " group by tid4 order by v desc";
	$q_paid_earnings_tid_combinations = "select COALESCE(SUM(payment),0) v,CONCAT(tid1,' ',tid2, ' ',tid3, ' ',tid4) n from idevaff_archive where id = $id and CONCAT(tid1,tid2,tid3,tid4) IS NOT NULL AND CONCAT(tid1,tid2,tid3,tid4) <>'' and top_tier_tag=0 and code " . $date_where .$whereCond. " group by n order by v desc";
	$q_paid_earnings_src1 = "select COALESCE(SUM(payment),0) v,src1,src2 from idevaff_archive where id = $id and top_tier_tag=0 and src1 IS NOT NULL AND src1 <> ''  and src2 IS NOT NULL AND src2 <> '' and code " . $date_where .$whereCond. " group by src1,src2 order by v desc";

	$q_approved_country = "select COUNT(*) v, geo n from idevaff_sales where approved=1 and id = $id and top_tier_tag=0 and code " . $date_where .$whereCond. " group by geo".str_replace('___','date_',$grouping)." order by v desc";
	$q_paid_country = "select COUNT(*) v, geo n from idevaff_archive where id = $id and top_tier_tag=0 and code " . $date_where .$whereCond. " group by geo".str_replace('___','date_',$grouping)." order by v desc";
	$q_approved_sub_id = "select COUNT(*) v, sub_id n from idevaff_sales where approved=1 and id = $id and sub_id IS NOT NULL AND sub_id <> ''  and top_tier_tag=0 and code " . $date_where .$whereCond. " group by sub_id order by v desc";
	$q_paid_sub_id = "select COUNT(*) v, sub_id n from idevaff_archive where id = $id and top_tier_tag=0  and sub_id IS NOT NULL AND sub_id <> '' and code " . $date_where .$whereCond. " group by sub_id order by v desc";
	$q_approved_tid1 = "select COUNT(*) v, tid1 n from idevaff_sales where approved=1 and id = $id and tid1 IS NOT NULL AND tid1 <> ''  and top_tier_tag=0 and code " . $date_where .$whereCond. " group by tid1 order by v desc";
	$q_paid_tid1 = "select COUNT(*) v, tid1 n from idevaff_archive where id = $id and top_tier_tag=0  and tid1 IS NOT NULL AND tid1 <> '' and code " . $date_where .$whereCond. " group by tid1 order by v desc";
	$q_approved_tid2 = "select COUNT(*) v, tid2 n from idevaff_sales where approved=1 and id = $id  and tid2 IS NOT NULL AND tid2 <> '' and top_tier_tag=0 and code " . $date_where .$whereCond. " group by tid2 order by v desc";
	$q_paid_tid2 = "select COUNT(*) v, tid2 n from idevaff_archive where id = $id and top_tier_tag=0  and tid2 IS NOT NULL AND tid2 <> '' and code " . $date_where .$whereCond. " group by tid2 order by v desc";
	$q_approved_tid3 = "select COUNT(*) v, tid3 n from idevaff_sales where approved=1 and id = $id  and tid3 IS NOT NULL AND tid3 <> '' and top_tier_tag=0 and code " . $date_where .$whereCond. " group by tid3 order by v desc";
	$q_paid_tid3 = "select COUNT(*) v, tid3 n from idevaff_archive where id = $id and top_tier_tag=0  and tid3 IS NOT NULL AND tid3 <> '' and code " . $date_where .$whereCond. " group by tid3 order by v desc";
	$q_approved_tid4 = "select COUNT(*) v, tid4 n from idevaff_sales where approved=1 and id = $id  and tid4 IS NOT NULL AND tid4 <> '' and top_tier_tag=0 and code " . $date_where .$whereCond. " group by tid4 order by v desc";
	$q_paid_tid4 = "select COUNT(*) v, tid4 n from idevaff_archive where id = $id and top_tier_tag=0  and tid4 IS NOT NULL AND tid4 <> '' and code " . $date_where .$whereCond. " group by tid4 order by v desc";
	$q_approved_tid_combinations = "select count(*) v,CONCAT(tid1,' ',tid2, ' ',tid3, ' ',tid4) n from idevaff_sales where id = $id and CONCAT(tid1,tid2,tid3,tid4) IS NOT NULL AND CONCAT(tid1,tid2,tid3,tid4) <>'' and approved=1 and top_tier_tag=0 and code " . $date_where .$whereCond. " group by n order by v desc";
	$q_paid_tid_combinations = "select count(*) v,CONCAT(tid1,' ',tid2, ' ',tid3, ' ',tid4) n from idevaff_archive where id = $id and CONCAT(tid1,tid2,tid3,tid4) IS NOT NULL AND CONCAT(tid1,tid2,tid3,tid4) <>'' and top_tier_tag=0 and code " . $date_where .$whereCond. " group by n order by v desc";
	$q_approved_src1 = "select COUNT(*) v, src1, src2 from idevaff_sales where approved=1 and id = $id  and src1 IS NOT NULL AND src1 <> '' and src2 IS NOT NULL AND src2 <> '' and top_tier_tag=0 and code " . $date_where .$whereCond. " group by src1,src2 order by v desc";
	$q_paid_src1 = "select COUNT(*) v, src1, src2 from idevaff_archive where id = $id and top_tier_tag=0  and src1 IS NOT NULL AND src1 <> '' and src2 IS NOT NULL AND src2 <> '' and code " . $date_where .$whereCond. " group by src1,src2 order by v desc";
//	echo $q_paid_country;
	...
	if ($info === "earnings" || $info === 'earnings_total' || $info === 'epc' || $info === 'epc_total') {
		...
		foreach ($db->query($q_earnings_src1) as $uv) {
			...
			if($src1 == 1){
				...
				foreach($banner_results as $banner_result) {
					$smart_tool['v'] = $uv['v'];
					$smart_tool['n'] = $banner_result['name']." ".$banner_result['description']." ".$banner_result['language']." ".$banner_result['size1']."x".$banner_result['size2']/*." - Fallback"*/;
...
try {
	$t = detailsByDateRange($startD, $endD, $current_date, $db, $linkid, $info,$country,$subId,$tId1,$tId2,$tId3,$tId4,$combinations,$smartTools,$offerTools,$grouping);
