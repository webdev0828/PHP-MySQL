...
if ( $res->rowCount() ) {
    foreach ( $res->fetchAll() as $pqry) {
        ...
        if ($second_contact == 1) {
			$show_user_info = $spaces . "<i class='fa fa-caret-right' aria-hidden='true'></i> <a href='mailto:".$temail."'>" . $tieruser . "</a>";
        } else {
            $show_user_info = $spaces . "<i class='fa fa-caret-right' aria-hidden='true'></i> " . $tieruser;
        }
        ...
        if ( $has_child ) {
            ...
            $child_space = $space + 5;
            $ret .= "<span class='pull-right'><button class='load_more' data-space='$child_space' data-parent='$tierid' data-page_now='0' data-token='$token'><i class='fa fa-plus' aria-hidden='true'></i></button></span>";
        }
