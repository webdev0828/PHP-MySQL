...
//$pay_details = $db->query("select amt from idevaff_paylevels where type = '$info1' and level = '$info2'");
    ...
    if ($info1 == 1) {
        ...
        if ($cur_sym_location == 1) {
            $payamt = $cur_sym . $payamt;
        }
        if ($cur_sym_location == 2) {
            $payamt = $payamt . " " . $cur_sym;
        }
...
/* slows down stats page $unchits is not used
...
// fake define end
    ...
    $get_notes = $db->prepare("select * from idevaff_notes where note_display = '1' and (note_to = '0' or note_to = ?) order by id DESC LIMIT 9");
    ...
    while ($r = $get_notes->fetch()) {
        ...
        $note_date = date($dateformat.' H:i', $r['stamp']);
        $note_to = $r['note_to'];
        ...
        if ($note_to == $linkid) {
            $note_to = "<font color=\"#CC0000\">" . $affiliate_username . "</font>";
        } else {
            $note_to = $general_notes_all;
        }
        $tmp = array(
            'note_date' => $note_date,
            'note_to' => $note_to,
...
if ($data_get == 6) {
    ...
    if ((isset($_POST['cut'])) && (is_numeric($_POST['cut']))) {
        if ($_POST['cut'] == '10') {
            $smarty->assign('cut_10', ' selected');
        }
        if ($_POST['cut'] == '25') {
            $smarty->assign('cut_25', ' selected');
        }
        if ($_POST['cut'] == '50') {
            $smarty->assign('cut_50', ' selected');
        }
        if ($_POST['cut'] == '100') {
            $smarty->assign('cut_100', ' selected');
        }
        if ($_POST['cut'] == '250') {
            $smarty->assign('cut_250', ' selected');
        }
        if ($_POST['cut'] == '500') {
            $smarty->assign('cut_500', ' selected');
        }
        $limit = $_POST['cut'];
    } else {
        $limit = '10';
...
//$lpast = $db->query("select record, code, payment, approved, sub_id from idevaff_sales where id = '$linkid' and approved = '1' and top_tier_tag = '0' ORDER BY code DESC");
            $lpast = $db->prepare("select record, code, payment, approved, sub_id, geo from idevaff_sales where id = ? and approved = '1' and top_tier_tag = '0' ORDER BY code DESC");
            $lpast->execute(array($linkid));
            if ($lpast->rowCount()) {
                $smarty->assign('commission_results_exist', 1);
            }
        }
...
//$lpast = $db->query("select record, code, payment, approved, sub_id from idevaff_sales where id = '$linkid' and approved = '1' and top_tier_tag = '1' ORDER BY code DESC");
            $lpast = $db->prepare("select record, code, payment, approved, sub_id, geo from idevaff_sales where id = ? and approved = '1' and top_tier_tag = '1' ORDER BY code DESC");
            $lpast->execute(array($linkid));
            if ($lpast->rowCount()) {
                $smarty->assign('commission_results_exist', 1);
            }
...
//$lpast = $db->query("select record, code, payment, approved, sub_id from idevaff_sales where id = '$linkid' and approved = '0' and delay = '0' and payment != '0' ORDER BY code DESC");
            $lpast = $db->prepare("select record, code, payment, approved, sub_id, geo from idevaff_sales where id = ? and approved = '0' and delay = '0' and payment != '0' ORDER BY code DESC");
            $lpast->execute(array($linkid));
            if ($lpast->rowCount()) {
                $smarty->assign('commission_results_exist', 1);
            }
...
//$lpast = $db->query("select record, code, payment, sub_id from idevaff_archive where id = '$linkid' and top_tier_tag = '0' ORDER BY code DESC");
            $lpast = $db->prepare("select record, code, payment, sub_id, geo from idevaff_archive where id = ? and top_tier_tag = '0' ORDER BY code DESC");
            $lpast->execute(array($linkid));
            if ($lpast->rowCount()) {
                $smarty->assign('commission_results_exist', 1);
            }
...
//$lpast = $db->query("select record, code, payment, sub_id from idevaff_archive where id = '$linkid' and top_tier_tag = '1' ORDER BY code DESC");
            $lpast = $db->prepare("select record, code, payment, sub_id, geo from idevaff_archive where id = ? and top_tier_tag = '1' ORDER BY code DESC");
            $lpast->execute(array($linkid));
            if ($lpast->rowCount()) {
                $smarty->assign('commission_results_exist', 1);
            }
...
comdetails_type_3,comdetails_type_4,comdetails_status_1,comdetails_status_2,comdetails_status_3,comdetails_not_available,sub_tracking_none';
        ...
        $geo = $getdeta['geo'];
        if (!$sub_id) {
            $sub_id = $sub_tracking_none;
        }
        if (!$tid1) {
            $tid1 = $account_not_available;
        }
        if (!$tid2) {
            $tid2 = $account_not_available;
        }
        if (!$tid3) {
            $tid3 = $account_not_available;
        }
        if (!$tid4) {
            $tid4 = $account_not_available;
        }
        if (!$geo) {
            $geo = $account_not_available;
        }
        ...
        $smarty->assign('commission_details_geo', $geo);
...
//  while ($available_countries = mysql_fetch_array($get_countries)) {
    ...
    if ($get_countries->rowCount()) {
        while ($available_countries = $get_countries->fetch()) {
            ...
            if ($email_table_extension == $pack_table) {
...
//$edit_details = $db->query("select f_name, l_name, address_1, address_2, city, state, phone, fax, zip, country, email, company, payable, url, tax_id_ssn, pay_method, paypal, stripe, pp from idevaff_affiliates where id = $linkid");
    ...
    if (isset($_POST['f_name'])) {
        $smarty->assign('postfname', html_output($_POST['f_name'], ENT_QUOTES, 'UTF-8'));
    } else {
        $smarty->assign('postfname', $f_name);
    }
    if (isset($_POST['state'])) {
        $smarty->assign('poststate', html_output($_POST['state'], ENT_QUOTES, 'UTF-8'));
    } else {
        $smarty->assign('poststate', $state);
    }
    if (isset($_POST['l_name'])) {
        $smarty->assign('postlname', html_output($_POST['l_name'], ENT_QUOTES, 'UTF-8'));
    } else {
        $smarty->assign('postlname', $l_name);
    }
    if (isset($_POST['phone'])) {
        $smarty->assign('postphone', html_output($_POST['phone'], ENT_QUOTES, 'UTF-8'));
    } else {
        $smarty->assign('postphone', $phone);
    }
    if (isset($_POST['address_one'])) {
        $smarty->assign('postaddr1', html_output($_POST['address_one'], ENT_QUOTES, 'UTF-8'));
    } else {
        $smarty->assign('postaddr1', $address_1);
    }
    if (isset($_POST['fax'])) {
        $smarty->assign('postfaxnm', html_output($_POST['fax'], ENT_QUOTES, 'UTF-8'));
    } else {
        $smarty->assign('postfaxnm', $fax);
    }
    if (isset($_POST['address_two'])) {
        $smarty->assign('postaddr2', html_output($_POST['address_two'], ENT_QUOTES, 'UTF-8'));
    } else {
        $smarty->assign('postaddr2', $address_2);
    }
    if (isset($_POST['zip'])) {
        $smarty->assign('postzip', html_output($_POST['zip'], ENT_QUOTES, 'UTF-8'));
    } else {
        $smarty->assign('postzip', $zip);
    }
    if (isset($_POST['city'])) {
        $smarty->assign('postcity', html_output($_POST['city'], ENT_QUOTES, 'UTF-8'));
    } else {
...
// if ($get_logo->fetch()) {
    ...
    if ($logo_type == 1) {
        $smarty->assign('logo_size_required', 1);
    }
...
//$get4 = $db->query("select distinct tid4 from idevaff_iptracking where tid4 != '' and acct_id = '$linkid'");
    ...
    if ((isset($_POST['custom_report'])) && ($_POST['custom_report'] == 1)) {
        ...
        if (($tid1) || ($tid2) || ($tid3) || ($tid4)) {
            ...
            if ($tid1 != "") {
                $sql_where .= "AND tid1 = '$tid1' AND";
            } else {
                $sql_groupby .= ",tid1,";
            }
            if ($tid2 != "") {
                $sql_where .= "AND tid2 = '$tid2' AND";
            } else {
                $sql_groupby .= ",tid2,";
            }
            if ($tid3 != "") {
                $sql_where .= "AND tid3 = '$tid3' AND";
            } else {
                $sql_groupby .= ",tid3,";
            }
            if ($tid4 != "") {
                $sql_where .= "AND tid4 = '$tid4' AND";
            } else {
                $sql_groupby .= ",tid4,";
            }
...
// echo $sql;
            ...
            if ($get_report->rowCount()) {
                ...
                $smarty->assign('no_results_found', 1);
            }
        } else {
            $smarty->assign('no_results_found', 1);
        }
...
if ((isset($_POST['sub_report'])) && ($_POST['sub_report'] == 1)) {
    ...
    if ((isset($_REQUEST['report'])) && ($_REQUEST['report'] == 1)) {
        $smarty->assign('commission_group_name', $details_drop_1);
        $lpast = $db->prepare("select record, code, payment, approved, geo from idevaff_sales where id = ? and approved = '1' and top_tier_tag = '0' ORDER BY code DESC");
        $lpast->execute(array($linkid));
        if ($lpast->rowCount()) {
            $smarty->assign('commission_results_exist', 1);
        }
    ...
    if ((isset($_REQUEST['report'])) && ($_REQUEST['report'] == 2)) {
        $smarty->assign('commission_group_name', $details_drop_2);
        $lpast = $db->prepare("select record, code, payment, approved, geo from idevaff_sales where id = ? and approved = '1' and top_tier_tag = '1' ORDER BY code DESC");
        $lpast->execute(array($linkid));
        if ($lpast->rowCount()) {
            $smarty->assign('commission_results_exist', 1);
        }
    ...
    if ((isset($_REQUEST['report'])) && ($_REQUEST['report'] == 3)) {
        $smarty->assign('commission_group_name', $details_drop_3);
        $lpast = $db->prepare("select record, code, payment, approved, geo from idevaff_sales where id = ? and approved = '0' and delay = '0' and payment != 0 ORDER BY code DESC");
        $lpast->execute(array($linkid));
        if ($lpast->rowCount()) {
            $smarty->assign('commission_results_exist', 1);
        }
    ...
    if ((isset($_REQUEST['report'])) && ($_REQUEST['report'] == 4)) {
        $smarty->assign('commission_group_name', $details_drop_4);
        $lpast = $db->prepare("select record, code, payment, geo from idevaff_archive where id = ? and top_tier_tag = '0' ORDER BY code DESC");
        $lpast->execute(array($linkid));
        if ($lpast->rowCount()) {
            $smarty->assign('commission_results_exist', 1);
        }
    ...
    if ((isset($_REQUEST['report'])) && ($_REQUEST['report'] == 5)) {
        $smarty->assign('commission_group_name', $details_drop_5);
        $lpast = $db->prepare("select record, code, payment, geo from idevaff_archive where id = ? and top_tier_tag = '1' ORDER BY code DESC");
        $lpast->execute(array($linkid));
        if ($lpast->rowCount()) {
            $smarty->assign('commission_results_exist', 1);
        }
    ...
    if ((isset($_REQUEST['report'])) && ($_REQUEST['report'] == 6)) {
        $smarty->assign('commission_group_name', $details_drop_6);
        $lpast = $db->prepare("select record, code, payment, approved, sub_id, geo from idevaff_sales where id = ? and approved = '0' and delay = '1' and payment != 0 ORDER BY code DESC");
        $lpast->execute(array($linkid));
        if ($lpast->rowCount()) {
            $smarty->assign('commission_results_exist', 1);
        }
