...
if (isset($_POST["edit"])) {
    ...
    $st->execute(array($_POST["group"], $_POST["d"], $_POST["alt_tag_edit"], $_POST["edit"]));
...
if (isset($_POST["add"])) {
    if(!is_dir("/var/www/sublimerevenue.com/media/banners/".$_POST["group"]."/".$_POST["desc"]."/".$_POST["language"]."/")) {
    mkdir("/var/www/sublimerevenue.com/media/banners/".$_POST["group"]."/".$_POST["desc"]."/".$_POST["language"]."/", 0755, true);
}
    $direct_url = $_POST["direct_url"];
    if ($direct_url != "" && $direct_url != "https://") {
        ...
        if (isset($test["mime"])) {
            ...
            $st = $db->prepare("insert into idevaff_banners (grp, size1, size2, image, description, alt_tag, sort, local, language) VALUES (?, ?, ?, ?, ?, ?, ?, '1', ?)");
            $st->execute(array($group, $s1, $s2, $direct_url, $bdesc, $alt_tag, $new_sort, $language));
            include "updates/auto_update_groups.php";
            $success_message = "<strong>Success!</strong> The <strong>".$group."</strong> <strong>".$bdesc."</strong> <strong>".$language."</strong> banner(s) has been added to the database and banner linking code has been created for your affiliates.";
        } else {
            $fail_message = "<strong>Error!</strong><br />- Invalid link to image file. Make sure the URL you're entering is pointing to a valid image file.<br />- For security purposes, iDevAffiliate only recognizes <strong>.gif</strong>, <strong>.jpg</strong> and <strong>.png files.<br /><br />Example URL: https://www.yoursite.com/images/banner.<strong>jpg</strong>";
        }
    } else {
        ...
        if (strlen(trim($_FILES["file"]["name"][$i])) < 1) {
            $fail = 1;
            $fail_message = "<strong>Error!</strong> Missing banner file name.";
        }
        if (!isset($fail) && file_exists($path . "/media/banners/".$_POST["group"]."/".$_POST["desc"]."/".$_POST["language"]."/" . $_FILES["file"]["name"][$i])) {
            $fail = 1;
            $fail_message = "<strong>Error!</strong> The filename already exists. Please rename your upload file and try again.";
        }
        if (!isset($fail)) {
            $res = copy($_FILES["file"]["tmp_name"][$i], (string) $path . "/media/banners/".$_POST["group"]."/".$_POST["desc"]."/".$_POST["language"]."/" . $_FILES["file"]["name"][$i]);
            if (!$res) {
                $fail = 1;
        ...
        if (!isset($fail) && is_uploaded_file($_FILES["file"]["tmp_name"][$i])) {
            if ($_FILES["file"]["type"][$i] == "image/gif" || $_FILES["file"]["type"][$i] == "image/jpeg" || $_FILES["file"]["type"][$i] == "image/pjpeg" || $_FILES["file"]["type"][$i] == "image/x-png" || $_FILES["file"]["type"][$i] == "image/png") {
                $fname = $_POST["group"]."/".$_POST["desc"]."/".$_POST["language"]."/".$_FILES["file"]["name"][$i];
                $group = $_POST["group"];
                ...
                $new_sort = $result["sort"] + 1;
                $language = $_POST["language"];
                $st = $db->prepare("insert into idevaff_banners (grp, size1, size2, image, description, alt_tag, sort, local, language) VALUES (?, ?, ?, ?, ?, ?, ?, '0', ?)");
                $st->execute(array($group, $s1, $s2, $fname, $bdesc, $alt_tag, $new_sort, $language));
                include "updates/auto_update_groups.php";
                $success_message = "<strong>Success!</strong> The <strong>".$group."</strong> <strong>".$bdesc."</strong> <strong>".$language."</strong> banner(s) has been added to the database and banner linking code has been created for your affiliates.";
            } else {
...
if (isset($_POST["sort_order"])) {
    ...
    if (is_numeric($sort_order)) {
        ...
        $st->execute(array($sort_order, $banner_id));
...
echo "</div>\r\n\r\n<div class=\"page-header\">\r\n<div class=\"page-title\"><h3>Banners</h3><span>Banners uploaded here are automatically made available to your affiliates.</span></div>\r\n";
include "templates/stats.php";
echo "</div>\r\n\r\n";
include "includes/notifications.php";
echo "\r\n<div class=\"tabbable tabbable-custom\">\r\n<ul class=\"nav nav-tabs\">\r\n<li ";
makeActiveTab(1);
echo "><a href=\"#tab_1_1\" data-toggle=\"tab\">Add A Banner</a></li>\r\n<li ";
makeActiveTab(2);
echo "><a href=\"#tab_1_2\" data-toggle=\"tab\">Manage Banners</a></li>\r\n<li ";
makeActiveTab(3);
...
makeActiveTab(1, "no");
echo "\" id=\"tab_1_1\">\r\n<div class=\"col-md-12\">\r\n\r\n";
if (!is_writable("../media/banners/".$_POST["group"]."/".$_POST["desc"]."/".$_POST["language"]."/")) {
    echo "<div class=\"alert alert-warning\" style=\"margin-top:20px;\"><h4>Folder Permissions Warning</h4>The <strong>/media/banners/".$_POST["group"]."/".$_POST["desc"]."/".$_POST["language"]."/</strong> folder requires <u>WRITE</u> permissions in order to upload a banner image. If you'd like to upload a banner image, please issue <u>write</u> permissions to the <strong>/media/banners/".$_POST["group"]."/".$_POST["desc"]."/".$_POST["language"]."/</strong> folder now. If you aren't planning to upload a banner image, you can ignore this message and use a direct URL instead.</div>\r\n";
}
echo "\r\n<div class=\"widget box\" style=\"margin-top:20px;\">\r\n<div class=\"widget-header\"><h4><i class=\"icon-picture\"></i> Add A New Banner</h4></div>\r\n<div class=\"widget-content\">\r\n<form class=\"form-horizontal row-border\" ENCTYPE=\"multipart/form-data\" method=\"post\" action=\"banners.php\">\r\n<input name=\"csrf_token\" value=\"";
echo $_SESSION["csrf_token"];
echo "\" type=\"hidden\" />\r\n<div class=\"form-group\">\r\n<label class=\"col-md-3 control-label\">Marketing Group</label>\r\n<div class=\"col-md-9\"><select class=\"form-control input-width-xlarge\" name=\"group\" style=\"display:inline-block;\">\r\n";
    if (isset($_POST['group'])) {
        echo "<option value='".$_POST['group']."' selected>From Last Submit</option>\n";
    }
...
echo "\"></div>\r\n</div>\r\n<div class=\"form-group\">\r\n<label class=\"col-md-3 control-label\"><b>Language (REQUIRED)</b></label>\r\n<div class=\"col-md-6\"><input type=\"text\" name=\"language\" class=\"form-control\" value=\"";
if (isset($_POST["language"])) {
    echo html_output($_POST["language"]);
}
echo "\" required></div>\r\n</div>\r\n<div class=\"form-group\">\r\n<label class=\"col-md-3 control-label\"><b>Banner Set (REQUIRED)</b></label>\r\n<div class=\"col-md-6\"><input type=\"text\" name=\"desc\" class=\"form-control\" value=\"";
if (isset($_POST["desc"])) {
    echo html_output($_POST["desc"]);
}
echo "\" required></div>\r\n</div>\r\n<div class=\"col-md-12 well\">Upload a banner image OR enter a direct URL to a banner image. Banner image types allowed are <font color=\"#CC0000\">GIF</font>, <font color=\"#CC0000\">JPG</font> and <font color=\"#CC0000\">PNG</font>.</div>\r\n<div class=\"form-group\">\r\n<label class=\"col-md-3 control-label\">Upload A Banner</label>\r\n<div class=\"col-md-9\"><input type=\"file\" name=\"file[]\" data-style=\"fileinput\" multiple/></div>\r\n</div>\r\n<div class=\"form-group\">\r\n<label class=\"col-md-3 control-label\">Enter URL To Banner Image</label>\r\n<div class=\"col-md-6\"><input type=\"text\" name=\"direct_url\" class=\"form-control\" value=\"https://\" /></div>\r\n</div>\r\n<div class=\"form-actions\">\r\n<input type=\"submit\" value=\"Add Banner(s)\" class=\"btn btn-primary\">\r\n</div>\r\n<input type=\"hidden\" name=\"add\" value=\"1\">\r\n</form>\r\n</div>\r\n</div>\r\n</div>\r\n</div>\r\n\r\n<div class=\"tab-pane";
makeActiveTab(2, "no");
...
if ($linklist/*->rowCount()*/) {
    ...
    while ($qry = $linklist->fetch()) {
        ...
        if ($local == 1) {
            ...
            $url_help = $base_url . "/media/banners/".$qry["grp"]."/".$qry["description"]."/".$qry["language"]."/" . $image;
        }
