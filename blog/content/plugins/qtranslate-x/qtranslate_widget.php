...
function qtranxf_generateLanguageSelectCode($args = array(), $id='') {
	...
	echo PHP_EOL.'<ul class="header-nav__social" id="'.$id.'">'.PHP_EOL;
	switch($type) {
		case 'image':
		case 'text':
		case 'css_only':
		case 'dropdown': {
			foreach(qtranxf_getSortedLanguages() as $language) {
				...
				echo '><img src="'.$flag_location.$q_config['flag'][$language].'" alt="'.$alt.'" class="lang_flag" style="margin-right:6.5px;margin-bottom:5px;"/>';
	...
	echo '</ul><div class="qtranxs_widget_end"></div><br>'.PHP_EOL;
}
